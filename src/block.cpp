...
Block::Block(int row, int column, char type, Power powerUp)
    : row(row), column(column), explodeTimer(0), type(type), powerUp(powerUp),
      rotation(0), powerUpRotate(0) {
...
void Block::Draw() {
  float scale = std::min(BLOCK_SIZE / grasstexture.width,
                         BLOCK_SIZE / grasstexture.height);

  DrawTextureEx(
      grasstexture,
      Vector2{(float)(BLOCK_SIZE * column), (float)(BLOCK_SIZE * row)}, 0.0f,
      scale, WHITE);
  ...
  case BRICK_BLOCK: {
    ...
    DrawTextureEx(
        tex, Vector2{(float)(BLOCK_SIZE * column), (float)(BLOCK_SIZE * row)},
        0.0f, scale, WHITE);
    break;
  }
  case CLAY_BLOCK: {
  ...
  case EXPLODE: {
    ...
    DrawTexturePro(tex, Rectangle{texturePart.x, texturePart.y, 32, 32},
                   Rectangle{(float)(BLOCK_SIZE * column) + BLOCK_SIZE / 2,
                             (float)(BLOCK_SIZE * row) + BLOCK_SIZE / 2,
                             (float)BLOCK_SIZE, (float)BLOCK_SIZE},
                   (Vector2){BLOCK_SIZE / 2, BLOCK_SIZE / 2}, rotation, WHITE);
...
void Block::Draw3d() {
  ...
  DrawModel(*grassModel,
            Vector3{column * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2 - 0.05f, 0.0f,
                    row * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2 - 0.05f},
            1.1, WHITE);
  ...
  case Power::SPEED: {
    ...
    DrawModelEx(*powerUpModel,
                ...
                {0.0f, 1.0f, 0.0f}, powerUpRotate, {.6f, .6f, .6f}, GOLD);
    break;
  }
  case Power::BOMB_RADIUS: {
    ...
    DrawModelEx(*powerUpModel,
                {column * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2,
                 (box.max.y - box.min.y) / 2,
                 row * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2},
                {0.0f, 1.0f, 0.0f}, powerUpRotate, {.6f, .6f, .6f}, BLUE);
    break;
  }
  case Power::BOMB_COUNT: {
    ...
    DrawModelEx(*powerUpModel,
                {column * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2,
                 (box.max.y - box.min.y) / 2,
                 row * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2},
                {0.0f, 1.0f, 0.0f}, powerUpRotate, {.4f, .4f, .4f}, WHITE);
    break;
  }
  default:
    break;
  ...
  case BRICK_BLOCK: {
    ...
    DrawModel(*block,
              Vector3{column * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2, 0.0f,
                      row * BLOCK_SIZE_3D + BLOCK_SIZE_3D / 2},
              1, WHITE);
...
bool Block::IsBrick() { return type == BRICK_BLOCK; }

bool Block::IsClay() { return type == CLAY_BLOCK; }

void Block::SetType(char type) { this->type = type; }

void Block::SetExplodeTimer() { explodeTimer = GetTime(); }
...
void Block::SetPowerType(Power power) {
  ...
  switch (power) {
  case Power::SPEED: {
    rect = {(float)((BLOCK_SIZE * column) + (BLOCK_SIZE * 0.2f)),
            (float)((BLOCK_SIZE * row) + (BLOCK_SIZE * 0.2f)),
            (float)(BLOCK_SIZE * .6), (float)(BLOCK_SIZE * .6)};
    break;
  }
  case Power::BOMB_RADIUS: {
    rect = {(float)((BLOCK_SIZE * column) + (BLOCK_SIZE * 0.1f)),
            (float)((BLOCK_SIZE * row) + (BLOCK_SIZE * 0.1f)),
            (float)(BLOCK_SIZE * .8), (float)(BLOCK_SIZE * .8)};
    break;
  }
  case Power::BOMB_COUNT: {
    rect = {(float)((BLOCK_SIZE * column) + (BLOCK_SIZE * 0.1f)),
